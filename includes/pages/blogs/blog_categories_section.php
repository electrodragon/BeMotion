<section class="blog-categories-section pt-5">
    <div class="categories-content-area">
        <div class="container">

            <div class="some-logo" data-aos="fade" data-aos-duration="1000" data-aos-easing="ease-in-out">
                <img src="/assets/images/blogs_page/logoless_box.png" alt="some logo">
            </div>

            <div class="row mt-3">
                <div class="col-12" data-aos="zoom-in" data-aos-duration="1000" data-aos-easing="ease-in-out">
                    <h1 class="text-center">Lorem ipsum dolor sit amet, congfs</h1>
                </div>
            </div>

            <div class="row justify-content-center mb-2">
                <div class="col-md-8">
                    <p class="text-center" data-aos="fade" data-aos-duration="1000" data-aos-easing="ease-in-out">
                        Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Lorem ipsum dolor sit amet,
                        consectetuer adipiscing elit. Lorem ipsum dolor sit amet, consectetuer adipiscing elit.
                    </p>
                </div>
            </div>

            <div class="row">
                <div class="col-12" data-aos="zoom-in" data-aos-duration="1000" data-aos-easing="ease-in-out">
                    <div class="categories-bar py-2 d-flex justify-content-center align-items-center flex-wrap">

                        <div class="categories-left">
                            <span class="category">ALL</span>
                            <span class="category">dladhlad</span>
                            <span class="category">alsdhlaad</span>
                            <span class="category">adnamdp adjad</span>
                            <input type="text" id="blogSearch" class="form-control form-control-sm ms-2" placeholder="Search blogs..." style="width: 200px;">
                            <div id="searchHeading" class="h5 mb-4 text-muted" style="display: none;"></div>
                        </div>
                    </div>
                </div>
            </div>

        </div>
    </div>
</section>